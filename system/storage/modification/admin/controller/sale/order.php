...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url, true)
				...
				$data['statistics'] = $this->url->link('sale/order/statistics', 'token=' . $this->session->data['token'], true);
	...
	public function history() {
		...
		//function Filtering feature characters
		private function cleanData($str){
			...
			$str= str_replace('"', '', $str);
			$str= str_replace('	', ' ', $str);
			$str = mb_convert_encoding($str, 'UTF-16LE', 'UTF-8');  // code table for Excel
			return $str;
		}

		//function Recording function in a file on one row
		private function fput_xls($out, $row_val) {
			//array_walk($row_val,  __NAMESPACE__ . '$this->cleanData');
			$arrlength = count($row_val);
			for($x = 0; $x < $arrlength; $x++) {
				$row_val[$x] = $this->cleanData($row_val[$x]) ;
			}
			//Separator File Export TAB
			fputcsv($out, $row_val , '	', '"');
		}
				
		// Функция за сравнение на дати
		private function comparedate($date1,$date2) {
				
				if ($date1 == ""){ return TRUE;}
				if ($date2 == ""){ return TRUE;}
				$date1 = str_replace ("-","",date("Y-m-d",strtotime($date1))) ;
				$date2 = str_replace ("-","",date("Y-m-d",strtotime($date2))) ;
		...
		public function statistics() {
			...
			//Start функции за отваряне на файл и инициализиране
			// filename for download
			$filename = "website_data_" . date('Y-m-d-h-i-s') . ".xls";
			//formed type of file 
			header("Content-Disposition: attachment; filename=\"$filename\"");
			header("Content-Type: application/vnd.ms-excel; charset=UTF-16LE");
			
			$out = fopen("php://output", 'w');
			//антетка и запис на антетка във файл
			$row_val = array ("order_id" , "status" , "user_id", "date_added" , "date_modified" , "customer" , "product_id" , "model" , "product_name" , "price" , "quantity" , "p_total","p_tax", "commission" , "user-seller" , "owner", "order_sub_total", "order_total", "o_total-sub_total", "o_taxes" , "o_shipping","shipping method","shipping city","payment method","payment city","currency code","email","telephone","store name");
			$this->fput_xls($out, $row_val);
			...
			if ($this->request->post['user_id'] != "") {
					...
					$sql_user_id = " AND p.user_id = " . $this->request->post['user_id'] ;
			}else{
					//echo "Полето user_id няма стойност";
					$sql_user_id = "" ;
			}
			
			if ($this->request->post['product_id'] != "") {
					//echo $this->request->post['product_id'] . ",";
					$sql_product_id = " AND p.product_id = " . $this->request->post['product_id'] ;
			}else{
					//echo "Полето product_id няма стойност";
					$sql_product_id = "" ;
			}
			
			if ($this->request->post['stat_order_status'] != "") {
					//echo $this->request->post['stat_order_status'] . ",";
					$sql_order_status = " AND os.order_status_id = '" . (int)$this->request->post['stat_order_status'] . "'" ;
						
			}else{
					//echo "Полето stat_order_status няма стойност";
					$sql_order_status = "" ;
			}
			...
			// Главно qry	
			$sql = "SELECT o.* , op.* , CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' " . $sql_order_status . ") AS status , op.total AS p_total, op.tax AS p_tax, p.user_id " . " FROM `" . DB_PREFIX . "order` o " ;
			$sql .= " LEFT JOIN (" . DB_PREFIX . "order_product op, " . DB_PREFIX . "product p ) ON (o.order_id= op.order_id AND op.product_id = p.product_id " . $sql_product_id . $sql_user_id . ")";
			//echo "<br>" . $sql ."<br>" ;	
	
			$query = $this->db->query($sql);
			$orders = $query->rows;
			$ordrder_old = -1; // маркер за предишен ордер
			foreach ($orders as $order) { // върти всички редове от заявката
				if (isset($order['order_id'])){ // филтрира некоректни ордери
				if (isset($order['status'])){	// филтрира некоректни статуси
				if ( $this->comparedate($order['date_added'],$this->request->post['start_date']) and $this->comparedate($this->request->post['end_date'],$order['date_added']) ){  // проверка на коректни дати
					$row_val[0] = $order['order_id'];
					$row_val[1] = $order['status'];
					$row_val[2] = $order['user_id'];
					...
					$row_val[5] = $order['customer'];
					$row_val[6] = $order['product_id'];
					$row_val[7] = $order['model'];
					$row_val[8] = $order['name'];
					$row_val[9] = $order['price'];
					$row_val[10] = $order['quantity'];
					$row_val[11] = $order['p_total'];
					$row_val[12] = $order['p_tax'];
					
					// всима комисионното от таблицата с потребителите
					$sql_commission = "SELECT commission FROM " . DB_PREFIX . "user WHERE user_id = " . $order['user_id'] ;
					$query_commission = $this->db->query($sql_commission);
					$commissions = $query_commission->rows;
					foreach ($commissions as $commission) {
							$row_val[13] = $commission['commission'];
					}
					
					// Изчислява сумите на продавача и собственика на база комисионно
					$row_val[14] = $order['p_total'] - $order['p_total'] * $commission['commission'] / 100;
					$row_val[14]= str_replace('TRUE	', '0', $row_val[14]);		// корекция за 0		
					
					$row_val[15] = $order['p_total'] * $commission['commission']/100;
					$row_val[15]= str_replace('TRUE	', '0', $row_val[15]);   // корекция за 0
					
					
					//проверка дали ордера е същият - ако е същият променливи от 16 до X са празни
					if (intval ($order['order_id']) != $ordrder_old ){
						//взима ордер субтотал
						$sql_total = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = " . $order['order_id'] . " AND code = 'sub_total'"  ;
						$query_total = $this->db->query($sql_total);
						$totals = $query_total->rows;
						foreach ($totals as $sub_total) {
								$row_val[16] = $sub_total['value'];
						}
						
						//взима ордер тотал
						$sql_total = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = " . $order['order_id'] . " AND code = 'total'"  ;
						$query_total = $this->db->query($sql_total);
						$totals = $query_total->rows;
						foreach ($totals as $total) {
								$row_val[17] = $total['value'];
						}
						
						// разлика между тотал и субтотал
						$row_val[18] = $total['value'] - $sub_total['value'];
						$row_val[18]= str_replace('TRUE	', '0', $row_val[18]); // корекция за 0
						
						// сумира всички текаси за ордера
						$sql_tax = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = " . $order['order_id'] . " AND code = 'tax'"  ;
						$query_tax = $this->db->query($sql_tax);
						$taxs = $query_tax->rows;
						$row_val[19] = "0.00";
						foreach ($taxs as $tax) {
								$row_val[19] = $row_val[19] + $tax['value'];
						}
						$row_val[19]= str_replace('TRUE	', '0', $row_val[19]);   // корекция за 0
						
						// всима стойност на доставката
						$sql_shipping = "SELECT value FROM " . DB_PREFIX . "order_total WHERE order_id = " . $order['order_id'] . " AND code = 'shipping'"  ;
						$query_shipping = $this->db->query($sql_shipping);
						$shippings = $query_shipping->rows;
						foreach ($shippings as $shipping) {
								$row_val[20] = $shipping['value'];
						}
						
						$row_val[21] = $order['shipping_method'];
						$row_val[22] = $order['shipping_city'];
						$row_val[23] = $order['payment_method'];
						$row_val[24] = $order['payment_city'];
						$row_val[25] = $order['currency_code'];
						$row_val[26] = $order['email'];
						$row_val[27] = "++" . $order['telephone'];
						$row_val[28] = $order['store_name'];
