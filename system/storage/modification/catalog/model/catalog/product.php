	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			...
			return array(
				...
				'tag'              => $query->row['tag'],
				'source_url'       => $query->row['source_url'],
...
'user_id'    => $query->row['user_id'],
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			$getuser_id = $this->db->escape(utf8_strtolower($data['filter_name']));
			$getuser_id = str_replace("merchant","",$getuser_id);
			$sql .= " OR LCASE(p.user_id) = '" . $getuser_id . "'";
